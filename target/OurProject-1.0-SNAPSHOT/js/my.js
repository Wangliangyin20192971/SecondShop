...
function editUser() {
    ...
    $.post(url, $("#editUser").serialize(), function (response) {
        window.location.href = getProjectPath()+"/selectNewUsers";
    })
}
...
function addSell() {
    ...
    if (ebid > 0) {}
        ...
        var url =getProjectPath()+ "/addSellGoods";
        $.post(url, $("#addSell").serialize(), function (response) {
            window.location.href = getProjectPath()+"/showSellGoods";
        })
    }
}

function deleteSell(id) {
    var url =getProjectPath()+ "/deleteSellGoods?goodsId=" + id;
    $.post(url, $("#editUser").serialize(), function (response) {
        window.location.href = getProjectPath()+"/showSellGoods";
    })
}
...
function deleteColl(id) {
    var url =getProjectPath()+ "/deleteCollection?collId=" + id;
    $.post(url, $("#editUser").serialize(), function (response) {
        window.location.href = getProjectPath()+"/showCollection";
    })
}

function confirmShipment(id) {
    var url = getProjectPath()+"/confirm?goodsId=" + id;
    $.get(url, function (response) {
        window.location.href = getProjectPath()+"/showSellGoods";
    })
}
...
function confirmReceipt(id) {
    var url = getProjectPath()+"/confirm?goodsId=" + id;
    $.get(url, function (response) {
        window.location.href = getProjectPath()+"/showBuyGoods";
    })
}

function collGoods(id) {
    var r = confirm("确认收藏?");
    if (r) {
        var url = getProjectPath()+"/collGoods?goodsId=" + id;
        $.get(url, function (response) {
            window.location.href = getProjectPath()+"/detail?id="+id;
        })
    }
}
...
function buyGoods(id) {
    var r = confirm("确认购买?");
    if (r) {
        var url = getProjectPath()+"/buyGoods?goodsId=" + id;
        $.get(url, function (response) {
            window.location.href = getProjectPath()+"/detail?id="+id;
        })
    }
